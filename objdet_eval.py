...
def measure_detection_performance(detections, labels, labels_valid, min_iou=0.5):
    ...
    for label, valid in zip(labels, labels_valid):
        ...
        if valid: # exclude all labels from statistics which are not considered valid
            ...
            box = label.box
            box_ = tools.compute_box_corners(box.center_x, box.center_y, box.width, box.length, box.heading)
            ## step 2 : loop over all detected objects
            for detection in detections:
                _, x, y, z,_,w, l, yaw = detection
                
                ## step 3 : extract the four corners of the current detection
                box_1 = tools.compute_box_corners(x,y,w,l,yaw)
                ## step 4 : computer the center distance between label and detection bounding-box in x, y, and z
                dis_x = box.center_x- x
                dis_y = box.center_y -y
                dis_z = box.center_z -z
                ## step 5 : compute the intersection over union (IOU) between label and detection bounding-box
                polygon_ = Polygon(box_)
                polygon_1 = Polygon(box_1)
                intersection = polygon_.intersection(polygon_1).area
                union = polygon_.union(polygon_1).area
